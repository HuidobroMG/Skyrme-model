...
Description:
    ...
    In this code I am solving the equation of motion for a generic B-skyrmion
    if the corresponding initial configuration is used. Since a general
    skyrmion does not have spherical symmetry in general the system
    is solved in the 3 cartesian dimensions, and we use a Gradient Flow method
    to obtain the minimal energy configuration.
    The system is solved in Skyrme units:
        E2MeV = 3*np.pi**2*fpi/e
        x2fm = hbarc/(fpi*e)
    
    Once the solution is obtained with some accepted accuracy the energy 
    density plot may be obtained using the Python library Mayavi.

This codes is just an example of a gradient flow method in Python, but
...
N -= 4
xmax = np.round((N-1)*dx/2, 3)
x = np.arange(-xmax, xmax+dx, dx)

# Plot the solution in 3 dimensions
Plot = False
if Plot == True:
    import mayavi.mlab as maylab
    # 3D PLOT
    
    # We create a scalar field with the module of Phi as the scalar
    src = maylab.pipeline.scalar_field(np.abs(ED))
    
    src.image_data.point_data.add_array(phi0[3].ravel().T)
    ...
    src2 = maylab.pipeline.set_active_attribute(src,
                                        point_scalars='scalar')
    
    # Cut isosurfaces of the norm
    contour = maylab.pipeline.contour(src2)
    
    # Now we select the 'angle' attribute, ie the phase of Phi
    contour2 = maylab.pipeline.set_active_attribute(contour,
                                        point_scalars='angle')
    
    # And we display the surface. The colormap is the current attribute: the phase.
    maylab.pipeline.surface(contour2, colormap='hsv')
    
    maylab.colorbar(title='Phase', orientation='vertical', nb_labels=3)