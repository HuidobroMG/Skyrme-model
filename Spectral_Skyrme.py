...
Description:
    ...
    in spherical coordinates using spectral methods. The hedgehog ansatz
    leads to a 1-dimensional second order differential equation which can
    be solved in two different domains. In this case only the quadratic
    and quartic terms of the Skyrme lagrangian are considered.
...
# First domain, r in [0, r_cut], x in [-1, 1]
# r1 = r_cut/2*(1 + x1)
N1 = 100
Ncoefs1 = 20
...
f1 = np.pi/(1+r1**2) # Initial configuration
df1 = -2*r1*np.pi/(1+r1**2)**2
...
# Second domain, r in [r_cut, inf], u in [1/r_cut, 0], x in [-1, 1]
# u = 1/r2 = (1 - x2)/(2*r_cut)
N2 = 100
Ncoefs2 = 20
